  ...
  ({ title, description, id, thumbnail, price, discountPercentage, tags, reviews, images }) => {
    return (
      <Card key={id} className="flex flex-col cursor-pointer">
        <CardHeader className="-m-6 text-green-400" onClick={() => showProduct(id)}>
          <img className="rounded-t-xl object-contain aspect-square" src={thumbnail} alt={title} />
        </CardHeader>
        <CardContent className="flex flex-col gap-2 mt-4 text-pink-400" onClick={() => showProduct(id)}>
          <article className="flex items-center gap-2">
            <p className="relative">
              <span className="absolute text-xs font-semibold left-0 top-1/2 -translate-y-1/2 text-blue-400">₹</span>
          ...
          <HoverCard>
            <HoverCardTrigger className="underline text-blue-500">View More</HoverCardTrigger>
            <HoverCardContent>
              <article className="flex flex-col gap-2">
                <h1 className="font-semibold text-red-400">{title}</h1>
